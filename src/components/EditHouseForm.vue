...
watchEffect(()=>{
  if(AppState.editable){
    editablehouseData.value = {...AppState.editable}
  }
})

async function editHouse() {
  try {
    await housesService.editHouse(editablehouseData.value, editablehouseData.value.id) // NOTE this is a ref object, we have to pass down the value stored inside of the ref
    ...
    Modal.getOrCreateInstance('#EdithouseFormModal').hide()
  } catch (error) {
    Pop.error(error)
  }
}
...
<template>
  ...
  <form @submit.prevent="editHouse()">
    ...
    <div class="form-floating mb-3">
      <input v-model="editablehouseData.id" type="text" class="form-control" id="id" placeholder="id..."
        required maxlength="500">
